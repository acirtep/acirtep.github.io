// built from https://github.com/55sketch/simple-rss/tree/master
  ...
  function handleJSON(data) {
    ...
    if (data.feed && data.items) {
      ...
      for (var i = 0; i < data.items.length; i++) {
        ...
        var pubdate = new Date(e.pubDate)

        var template = '<' + titleWrapper + '><a href="' + e.link + '" style="color: whitesmoke">' + pubdate.toLocaleDateString() + ' - ' + e.title + '</a></' + titleWrapper + '>' ;

        if (addLink === 'false') {

          template = '<' + titleWrapper + '>' + e.title + '</' + titleWrapper + '>';
